...
class AutoOffsetState extends MusicBeatState
{
	...
	var alreadyTapped:Map<Int, Bool> = [];
	...
	override function create()
	{
		...
		musicStream = new AudioStreamThing(Paths.music("offsetSong" + (easterEgg ? "-pixel" : "")));
		...
		FlxG.stage.addEventListener(KeyboardEvent.KEY_UP, keyUp, false);
	...
	function keyDown(e:KeyboardEvent)
	{
		...
		if (alreadyTapped[e.keyCode] == null || !alreadyTapped[e.keyCode])
		{
			alreadyTapped[e.keyCode] = true;
			if (e.keyCode == Keyboard.ENTER || e.keyCode == Keyboard.ESCAPE)
				return;
			if ((musicStream.time >= 4500 && musicStream.time <= 43500) && !(ending || endOfSong))
			{
				hitBeat();
			}
		}
	...
	{
		hitBeats++;
		var offsetAdd = Std.int(musicStream.time % 600);
